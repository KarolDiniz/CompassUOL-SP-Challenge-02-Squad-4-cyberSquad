...
class OrderServiceTest {
    ...
    void createOrder_ValidOrder_ReturnsCreatedOrder() {
        ...
        assertEquals(OrderStatus.CRIADO, createdOrder.getStatus());
    ...
    @Test
    void createOrder_InvalidProduct_ThrowsIllegalArgumentException() {
        Order order = new Order();
        List<OrderItem> items = new ArrayList<>();
        ...
        when(customerRepository.existsById(order.getCustomerId())).thenReturn(true);
        when(productRepository.existsById(item.getProductId())).thenReturn(false);
